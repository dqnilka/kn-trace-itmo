...
ОТБОР ПОНЯТИЙ (важно):
- Список понятий на входе собран по заданиям и может содержать ЧУЖЕРОДНЫЕ для темы термины (из соседних тем). Включай только то, что относится к ЯДРУ темы. Если понятие не вписывается в тему — НЕ упоминай его вовсе.
- Бери 4–6 действительно центральных понятий, не больше.

ГЛУБИНА — РАСКРЫВАЙ, А НЕ ПЕРЕЧИСЛЯЙ:
- Каждое основание классификации не просто называй, а поясняй: что это значит, в чём разница между полюсами, 1 короткий пример. Списки «А и Б, В и Г» без объяснения — запрещены.
- Цель — чтобы после чтения студент РЕАЛЬНО понял тему и решил задание, а не увидел оглавление.
...
СТРУКТУРА — АДАПТИВНАЯ (строгий Markdown, без заголовков # и ##; секции выделяй **жирным**, используй списки). Объём: для содержательной темы 400–550 слов (не экономь на пояснениях); совсем простую — короче. Блоки:

**Суть и классификация** — что это за тема и по каким основаниям делится предмет; КАЖДОЕ основание — с пояснением и примером. Это ядро разбора.

**Ключевые понятия** — 4–6 центральных терминов: **термин** — понятное определение + чем отличается от близких + когда применяется.

**На что обратить внимание / ловушки** — КОНКРЕТНЫЕ типовые ошибки: какие именно пары понятий путают и в чём, на какие слова в формулировках смотреть. Не общие фразы «важно понимать различия».

**Запомнить** — 3–5 тезисов, которые НЕ повторяют дословно блок «Суть»: дай практические выводы/различения, полезные на экзамене.

Блок с разбором конкретного примера задания добавляй ТОЛЬКО если он реально проясняет тему.

ТОН: русский, как хороший репетитор — живо, по делу, уверенно; без воды, без «играет важную роль в экономике», без «как видно из контекста», без канцелярита."""
